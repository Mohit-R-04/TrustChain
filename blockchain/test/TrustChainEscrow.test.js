...
describe("TrustChainEscrow", function () {
  ...
  describe("Fund Deposit", function () {
    ...
    it("Should deposit funds successfully", async function () {
      await expect(
        contract.connect(donor).depositFunds(SCHEME_ID, { value: DEPOSIT_AMOUNT })
      )
    ...
    it("Should track donor contribution", async function () {
      await contract.connect(donor).depositFunds(SCHEME_ID, { value: DEPOSIT_AMOUNT });
    ...
    it("Should prevent zero deposit", async function () {
      await expect(
        contract.connect(donor).depositFunds(SCHEME_ID, { value: 0 })
      ).to.be.revertedWith("no value");
    });

    it("Should prevent deposit to non-existent scheme", async function () {
      await expect(
        contract.connect(donor).depositFunds(999, { value: DEPOSIT_AMOUNT })
  ...
  describe("Payment Release", function () {
    ...
    it("Should release payment to vendor", async function () {
      ...
      await expect(contract.releasePayment(SCHEME_ID, MILESTONE_ID))
    ...
    it("Should prevent non-owner from releasing payment", async function () {
      await expect(contract.connect(donor).releasePayment(SCHEME_ID, MILESTONE_ID)).to.be.revertedWith(
        "not owner"
      );
    });

    it("Should prevent double payment", async function () {
      await contract.releasePayment(SCHEME_ID, MILESTONE_ID);
      await expect(contract.releasePayment(SCHEME_ID, MILESTONE_ID)).to.be.revertedWith(
        "not approved"
      );
    });
    ...
    it("Should prevent payment for non-approved milestone", async function () {
      ...
      await expect(contract.releasePayment(SCHEME_ID, milestone2)).to.be.revertedWith(
        "not approved"
      );
    });
  });
  ...
  describe("Refund Logic", function () {
    ...
    it("Should refund rejected milestone", async function () {
      ...
      await expect(contract.refundIfRejected(SCHEME_ID, MILESTONE_ID, donor.address))
    ...
    it("Should prevent non-owner from refunding", async function () {
      await expect(
        contract.connect(donor).refundIfRejected(SCHEME_ID, MILESTONE_ID, donor.address)
      ).to.be.revertedWith("not owner");
    });
    ...
    it("Should prevent refunding non-rejected milestone", async function () {
      ...
      await contract.createMilestone(SCHEME_ID, milestone2, MILESTONE_AMOUNT);
      await expect(
        contract.refundIfRejected(SCHEME_ID, milestone2, donor.address)
  ...
  describe("Reentrancy Protection", function () {
    it("Should protect against reentrancy in releasePayment", async function () {
      ...
      await expect(contract.releasePayment(SCHEME_ID, MILESTONE_ID)).to.not.be.reverted;
    });
  });
