...
- **Purpose**: Donors deposit funds on-chain into the escrow
...
- **Notes**:
  - Deposits are blocked after `lockFunds(schemeId)` is called.
  - There is also an overloaded function `depositFunds(uint256 schemeId, uint256 amount)` that requires `msg.value == amount`.
- **Example**:
  ```javascript
  const tx = await contract["depositFunds(uint256)"](1, { value: ethers.utils.parseEther("1") });
  await tx.wait();
  ```
...
- **Example**:
  ...
  await contract.createMilestone(1, 1, ethers.utils.parseEther("5"));
  ```
...
### Listen to Events (Backend/Frontend)
...
blockchain.private-key=${TRUSTCHAIN_ESCROW_PRIVATE_KEY}
```

**4. Web3j Integration (Implemented in this repo)**

Backend integration is implemented using Web3j (no generated wrapper required):

- REST endpoints: [BlockchainController.java](file:///Users/mohitreddy/Documents/TrustChain/backend/src/main/java/com/trustchain/backend/controller/BlockchainController.java)
- Client for owner/admin calls: [TrustChainEscrowClient.java](file:///Users/mohitreddy/Documents/TrustChain/backend/src/main/java/com/trustchain/backend/service/blockchain/TrustChainEscrowClient.java)
- Event sync (logs → DB): [BlockchainEventSyncService.java](file:///Users/mohitreddy/Documents/TrustChain/backend/src/main/java/com/trustchain/backend/service/blockchain/BlockchainEventSyncService.java)

**5. Event Listeners (Implemented in this repo)**

The backend polls contract logs and stores them in the `blockchain_event` table (JPA entity: [BlockchainEvent.java](file:///Users/mohitreddy/Documents/TrustChain/backend/src/main/java/com/trustchain/backend/model/BlockchainEvent.java)). Use `GET /api/blockchain/events/recent` to retrieve recent events for the UI.
