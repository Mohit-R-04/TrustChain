...
async function main() {
  ...
  try {
    ...
    console.log("═════════════════════════════════════════");
    const depositAmount = hre.ethers.utils.parseEther("1"); // 1 POL
    tx = await contract["depositFunds(uint256)"](schemeId, { value: depositAmount });
    await tx.wait();
    console.log(`✅ Funds deposited: ${hre.ethers.utils.formatEther(depositAmount)} POL`);

    // Verify balance
    const balance = await contract.getSchemeBalance(schemeId);
    console.log(`💰 Scheme balance: ${hre.ethers.utils.formatEther(balance)} POL\n`);
    ...
    console.log("═════════════════════════════════════════");
    const milestoneId = 1;
    const milestoneAmount = hre.ethers.utils.parseEther("0.5");
    tx = await contract.createMilestone(schemeId, milestoneId, milestoneAmount);
    await tx.wait();
    console.log(`✅ Milestone created: ID=${milestoneId}, Amount=${hre.ethers.utils.formatEther(milestoneAmount)} POL`);
    ...
    console.log("═════════════════════════════════════════");
    tx = await contract["releasePayment(uint256,uint256)"](schemeId, milestoneId);
    await tx.wait();
    console.log(`✅ Payment released to vendor: ${hre.ethers.utils.formatEther(milestoneAmount)} POL`);
